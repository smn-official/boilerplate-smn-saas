...
Dez agentes especializados. As definições vivem em [../agents/](../agents/) e as 50 skills, num
...
| `frontend-agent` | Tailwind + Vite + TypeScript sobre Razor | 5 |
...
| `cor-agent` | Cor a partir do contexto do produto | 1 |
...
| ViewModel e View da mesma tela | `net10-agent` (ViewModel), `frontend-agent` (View, estilo, TS) | ViewModel é C# de apresentação |
...
| Definir a cor primária do produto | `cor-agent` decide, `frontend-agent` aplica | Escolha é estratégia; aplicação é código |
...
2. `net10-agent` / `pgproc-agent` / `frontend-agent` — implementação.
